...
from sptr import to_3d_numpy, SparseTrTensor, sparse_self_attention, get_indices_params
...
def cart2sphere(xyz):
    ...
    # which x, y make max and min theta
...
def exponential_split(xyz, index_0, index_1, relative_position_index, a=0.05*0.25):
    ...
    idx = idx + ((3 * (2 ** (torch.div(idx, 2, rounding_mode='trunc'))) - 2) * a <= rel_pos_abs).float()
